...
const MAX_ONNX_PROTO_BYTES = 512 * 1024 * 1024;
...
export async function inspectOnnxModel(
  ...
  const modelDirectory = dirname(resolve(modelPath));
...
async function inspectExternalDataFiles(
  ...
  for (const location of [...rangesByLocation.keys()].sort()) {
    ...
    if (
      ...
      throw new Error(`external-data location escapes model directory: ${location}`);
    }
    ...
    for (const [, end] of ranges) {
      if (end > fileStat.size) {
        throw new Error(
          `external-data range exceeds ${location}: ${end} > ${fileStat.size}`,
        );
      }
    }
    files.push({
